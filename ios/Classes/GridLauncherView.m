...
static const CGFloat kGridLauncherViewMargin = 0;
static const CGFloat kGridLauncherViewPadding = 0;
static const CGFloat kGridLauncherViewPagerHeight = 20;
static const CGFloat kGridLauncherViewWobbleRadians = 1.5;
static const CGFloat kGridLauncherViewSpringLoadFraction = 0.18;
static const NSTimeInterval kGridLauncherViewEditHoldTimeInterval = 1;
...
@implementation GridLauncherScrollView
...
@implementation GridLauncherView

@synthesize columnCount, rowCount, delegate, itemHeight, contentSize, scrollType, showsVerticalScrollIndicator, showsHorizontalScrollIndicator, pagingEnabled, pagerEnabled, editable;
...
- (id)initWithFrame:(CGRect)frame withRowCount:(int)newRowCount withColumnCount:(int)newColumnCount withProxy:(TiProxy *)proxy
{
  if ((self = [super initWithFrame:frame])) {
    ...
    if (![proxy valueForKey:@"showVerticalScrollIndicator"]){
         self.showsVerticalScrollIndicator = NO;
    }
    if (![proxy valueForKey:@"showHorizontalScrollIndicator"]){
          self.showsHorizontalScrollIndicator = NO;
    }
    if (![proxy valueForKey:@"scrollType"]){
        self.scrollType = @"horizontal";
    }
    else {
        self.scrollType = [[proxy valueForKey:@"scrollType"] stringValue];
    }
    if (![proxy valueForKey:@"pagingEnabled"]){
        self.pagingEnabled = YES;
    }
    else {
        self.pagingEnabled = [[proxy valueForKey:@"pagingEnabled"] boolValue];
    }
      
      
    renderingButtons = NO;

    scrollView = [[GridLauncherScrollView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height - kGridLauncherViewPagerHeight - 30)];
    ...
    if ([self.scrollType isEqualToString:@"horizontal"]){
...
- (GridLauncherButton *)addButtonWithItem:(GridLauncherItem *)item
{
  GridLauncherButton *button = [[GridLauncherButton alloc] initWithFrame:item.view.bounds];
...
//    }
//    else {
//        return MAX(33, (scrollView.frame.size.width / columnCount));
//    }
}
...
- (NSInteger)currentPageIndex
{
    if ([self.scrollType isEqualToString:@"horizontal"]){
        ...
        return floor(scrollView.contentOffset.y / scrollView.frame.size.height);
    }
}

- (NSMutableArray *)pageWithItem:(GridLauncherItem *)item
{
...
- (NSIndexPath *)indexPathOfItem:(GridLauncherItem *)item
{
...
- (NSMutableArray *)pageWithButton:(GridLauncherButton *)button
{
...
- (void)setCurrentPageIndex:(NSInteger)pageIndex
{
    if ([self.scrollType isEqualToString:@"horizontal"]){
        ...
        scrollView.contentOffset = CGPointMake(0,scrollView.frame.size.height * pageIndex);
    }
}
...
- (void)layoutButtons
{
  ...
  CGFloat buttonWidth = ceil((self.frame.size.width - (kGridLauncherViewMargin * 2 + kGridLauncherViewPadding * (self.columnCount - 1))) / self.columnCount);
  CGFloat buttonHeight;
  buttonHeight = [self rowHeight];
...
//  }
  ...
  if ([self.scrollType isEqualToString:@"horizontal"]){
      CGFloat x = kGridLauncherViewMargin, minX = 0;
        
      for (NSMutableArray *buttonPage in buttons) {
        CGFloat y = kGridLauncherViewMargin;
        for (GridLauncherButton *button in buttonPage) {
          ...
          x += buttonWidth + kGridLauncherViewPadding;
            

            
          if (x >= minX + pageWidth) {
            y += buttonHeight + kGridLauncherViewPadding;
            x = minX + kGridLauncherViewMargin;
          }
      ...
      CGFloat y = kGridLauncherViewMargin, minY = 0, minX = 0;

      for (NSMutableArray *buttonPage in buttons) {
          CGFloat x = kGridLauncherViewMargin;
...
- (void)recreateButtons
{
  ...
  for (NSArray *page in pages) {
    ...
    for (GridLauncherItem *item in page) {
        GridLauncherButton *button = [self addButtonWithItem:item];
...
- (void)scrollToItem:(GridLauncherItem *)item animated:(BOOL)animated
{
  ...
  if (path != nil) {
    ...
    CGFloat x;
    if ([self.scrollType isEqualToString:@"horizontal"]){
         x = page * scrollView.frame.size.width;
        [scrollView setContentOffset:CGPointMake(x, 0) animated:animated];
    }
    else {
         x = page * scrollView.frame.size.height;
        [scrollView setContentOffset:CGPointMake(0, x) animated:animated];
    }
  }
}
...
- (void)layoutSubviews
{
  ...
  pager.frame = CGRectMake(0, scrollView.frame.size.height, self.frame.size.width, kGridLauncherViewPagerHeight);
...
- (void)startDraggingButton:(GridLauncherButton *)button withEvent:(UIEvent *)event
{
  ...
  [UIView setAnimationDuration:kGridLauncherViewFastTransitionDuration];
...
- (void)deselectButton:(GridLauncherButton *)button
{
  [button setSelected:NO];
}

- (void)buttonTouchedUpInside:(GridLauncherButton *)button
{
  if (editing) {
    ...
    [self performSelector:@selector(deselectButton:) withObject:button afterDelay:kGridLauncherViewTransitionDuration];
...
- (void)buttonTouchedUpOutside:(GridLauncherButton *)button
{
  if (editing) {
    if (button == dragButton) {
      [self startDraggingButton:nil withEvent:nil];
...
- (void)wobble
{
  ...
  if (editing) {
    CGFloat rotation = (kGridLauncherViewWobbleRadians * M_PI) / 180.0;
    ...
    if (nWobblyButtons >= 1) {
      [UIView setAnimationDuration:kGridLauncherViewWobbleTime];
      [UIView setAnimationDelegate:self];
      [UIView setAnimationDidStopSelector:@selector(wobble)];
      wobblesLeft = !wobblesLeft;
    } else {
      [NSObject cancelPreviousPerformRequestsWithTarget:self];
      [self performSelector:@selector(wobble) withObject:nil afterDelay:kGridLauncherViewWobbleTime];
    }
...
- (GridLauncherButton *)buttonForItem:(GridLauncherItem *)item
{
  NSIndexPath *path = [self indexPathOfItem:item];
  if (path) {
    NSInteger pageIndex = [path indexAtPosition:0];
...
- (void)removeItem:(GridLauncherItem *)item animated:(BOOL)animated
{
  ...
  if (itemPage) {
      GridLauncherButton *button = [self buttonForItem:item];
...
- (void)closeButtonTouchedUpInside:(GridLauncherButton *)closeButton
{
  for (NSArray *buttonPage in buttons) {
    for (GridLauncherButton *button in buttonPage) {
...
- (void)endEditing
{
  ...
  [UIView beginAnimations:nil context:nil];
  [UIView setAnimationDuration:kGridLauncherViewTransitionDuration];
  [UIView setAnimationDelegate:self];
...
- (void)editHoldTimer:(NSTimer *)timer
{
  ...
  NSArray *data = timer.userInfo;
    GridLauncherButton *button = [data objectAtIndex:0];
...
- (void)buttonTouchedDown:(GridLauncherButton *)button withEvent:(UIEvent *)event
{
  if (editing) {
    if (!dragButton) {
      [self startDraggingButton:button withEvent:event];
    ...
    TI_INVALIDATE_TIMER(editHoldTimer);
    if (editable) {
      editHoldTimer = [NSTimer scheduledTimerWithTimeInterval:kGridLauncherViewEditHoldTimeInterval
                                                       target:self
...
- (void)updatePagerWithContentOffset:(CGPoint)contentOffset
{
    if ([self.scrollType isEqualToString:@"horizontal"]){
        ...
        NSInteger oldPageNo = pager.currentPage;
        pager.currentPage = floor((contentOffset.y - pageWidth / 2) / pageWidth) + 1;
        if (oldPageNo != pager.currentPage) {
          if ([delegate respondsToSelector:@selector(launcherView:didChangePage:)]) {
            [delegate launcherView:self didChangePage:[NSNumber numberWithInteger:pager.currentPage]];
          }
    ...
    if ([self.scrollType isEqualToString:@"horizontal"]){
        if ([(NSNumber *)timer.userInfo boolValue]) {
          ...
          if (newX >= 0) {
            ...
            positionOrigin = -1;
            springing = YES;
            [self performSelector:@selector(springingDidStop) withObject:nil afterDelay:0.2];
          }
        } else {
          CGFloat newX = scrollView.contentOffset.x + scrollView.frame.size.width;
        ...
        if ([(NSNumber *)timer.userInfo boolValue]) {
          CGFloat newX = scrollView.contentOffset.y - scrollView.frame.size.height;
          ...
          if (newX >= 0) {
            CGPoint offset = CGPointMake(0, newX);
            [scrollView setContentOffset:offset animated:YES];
            [self updatePagerWithContentOffset:offset];
            dragOrigin.y += scrollView.frame.size.height;
            positionOrigin = -1;
            springing = YES;
            [self performSelector:@selector(springingDidStop) withObject:nil afterDelay:0.2];
          }
          ...
          CGFloat newX = scrollView.contentOffset.y + scrollView.frame.size.height;
          if (newX <= scrollView.contentSize.height - scrollView.frame.size.height) {
            CGPoint offset = CGPointMake(0, newX);
            [scrollView setContentOffset:offset animated:YES];
            [self updatePagerWithContentOffset:offset];
            dragOrigin.y -= scrollView.frame.size.height;
...
- (void)updateTouch
{
    if ([self.scrollType isEqualToString:@"horizontal"]){
       ...
       // NSLog(@"[ERROR] updateTouch x column row itemIndex  %f %i %i %i: ",x,column,row,itemIndex);
        ...
        CGFloat springLoadDistance = dragButton.frame.size.width * kGridLauncherViewSpringLoadFraction;
        ...
        if (goToPreviousPage || goToNextPage) {
          if (!springLoadTimer) {
            springLoadTimer = [NSTimer scheduledTimerWithTimeInterval:kGridLauncherViewSpringLoadTimeInterval
                                                               target:self
       ...
       // NSLog(@"[ERROR] updateTouch button origin %f %f: ",origin.x,origin.y);
       // NSLog(@"[ERROR] updateTouch button dragOrigin %f %f: ",dragOrigin.x,dragOrigin.y);

        
        CGFloat x = origin.x - scrollView.contentOffset.x;
        CGFloat y = origin.y - scrollView.contentOffset.y;

        NSInteger column = round(x / dragButton.frame.size.width);
        NSInteger row = round(y / dragButton.frame.size.height);
        
        NSInteger pageIndex = MAX(floor(scrollView.contentOffset.y / scrollView.frame.size.height), 0);
       
        NSInteger itemIndex = (row * self.columnCount) + column;
        
       // NSLog(@"[ERROR] updateTouch button x pageIndex %f %i: ",x ,pageIndex);
       // NSLog(@"[ERROR] updateTouch x column row itemIndex  %f %i %i %i: ",x,column,row,itemIndex);
        ...
        CGFloat springLoadDistance = dragButton.frame.size.height * kGridLauncherViewSpringLoadFraction;
        BOOL goToPreviousPage = dragButton.center.y - springLoadDistance < 0;
        BOOL goToNextPage = ((scrollView.frame.size.height - dragButton.center.y) - springLoadDistance) < 0;
        if (goToPreviousPage || goToNextPage) {
          if (!springLoadTimer) {
            springLoadTimer = [NSTimer scheduledTimerWithTimeInterval:kGridLauncherViewSpringLoadTimeInterval
                                                               target:self
...
- (void)pageChanged
{
  if ([delegate respondsToSelector:@selector(launcherView:didChangePage:)]) {
    ...
    if ([self.scrollType isEqualToString:@"horizontal"]){
        ...
        scrollView.contentOffset = CGPointMake(0,pager.currentPage * scrollView.frame.size.height);
    }
}
