...
// in simulator we redefine to format for testapp Developer console
...
#define DEFINE_EXCEPTIONS                                                                                      \
  ...
  {                                                                                                            \
    NSString *exceptionName = [@"org.testapp." stringByAppendingString:NSStringFromClass([self class])];  \
    TiExceptionThrowWithNameAndReason(exceptionName, reason, subreason, location);                             \
  }                                                                                                            \
                                                                                                               \
  +(void)throwException : (NSString *)reason subreason : (NSString *)subreason location : (NSString *)location \
  {                                                                                                            \
    NSString *exceptionName = @"org.testapp";                                                             \
    TiExceptionThrowWithNameAndReason(exceptionName, reason, subreason, location);                             \
  }
...
enum {
  UItestappNativeItemNone = -1,
  UItestappNativeItemSpinner = -2,
  UItestappNativeItemProgressBar = -3,

  UItestappNativeItemSlider = -4,
  UItestappNativeItemSwitch = -5,
  UItestappNativeItemMultiButton = -6,
  UItestappNativeItemSegmented = -7,

  UItestappNativeItemTextView = -8,
  UItestappNativeItemTextField = -9,
  UItestappNativeItemSearchBar = -10,

  UItestappNativeItemPicker = -11,
  UItestappNativeItemDatePicker = -12,

  UItestappNativeItemInfoLight = -13,
  UItestappNativeItemInfoDark = -14,

  UItestappNativeItemDisclosure = -15,

  UItestappNativeItemContactAdd = -16
};
...
/**
 *	TiThreadPerformOnMainThread should replace all testapp instances of
