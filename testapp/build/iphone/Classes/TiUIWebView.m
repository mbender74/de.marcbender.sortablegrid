...
- (WKWebView *)webView
{
  if (_webView == nil) {
    ...
    [controller addUserScript:[self userScripttestappInjectionForAppEvent]];
...
- (WKUserScript *)userScripttestappInjectionForAppEvent
{
  if (_pageToken == nil) {
  ...
  NSString *_testapp = [NSString stringWithFormat:@"%@%s", @"Ti", "tanium"];
  ...
  NSString *sourceString = [NSString stringWithFormat:source, _testapp, _pageToken, baseInjectScript];
...
- (WKUserScript *)userScripttestappJSEvaluationFromString:(NSString *)string
{
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(nonnull WKNavigationAction *)navigationAction decisionHandler:(nonnull void (^)(WKNavigationActionPolicy))decisionHandler
{
  ...
  if ([allowedURLSchemes containsObject:navigationAction.request.URL.scheme]) {
    if ([[UIApplication sharedApplication] canOpenURL:navigationAction.request.URL]) {
      // Event to return URL to testapp in order to handle OAuth and more
